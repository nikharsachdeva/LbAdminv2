...
@AndroidEntryPoint
class AddPackageFragment : BaseBottomSheet() {
    ...
    private fun onClick() {
        ...
        binding.addPackageBtn.setOnClickListener {
            if (isValid()) {
                var packagePayload = AddPackagePayload(
                    description = binding.packageDescTiet.text.toString(),
                    details = profileViewModel.benefitArray.value,
                    grossPrice = binding.priceTiet.text.toString().toDouble(),
                    price = binding.discountTiet.text.toString().toDouble(),
                    name = binding.packageNameTiet.text.toString(),
                    validity = binding.validityTiet.text.toString().toInt()
                )
                profileViewModel.addPackage(packagePayload)
            }
        }
    }
    ...
    private fun isValid(): Boolean {
        ...
        if (binding.packageNameTiet.text.toString().isNullOrEmpty()) {
            valid = false
            Toast.makeText(requireContext(), "Package name is required", Toast.LENGTH_SHORT).show()
        }

        if (binding.packageDescTiet.text.toString().isNullOrEmpty()) {
            valid = false
    ...
    private fun initObserver() {
        ...
        profileViewModel.benefitArray.observe(viewLifecycleOwner, Observer {
            if (it.isNullOrEmpty()) {
                binding.addPackageBtn.makeButtonDisabled()
            } else {
                ...
                binding.addPackageBtn.makeButtonEnabled()
            }
        })

        profileViewModel.addPackageLiveData.observe(viewLifecycleOwner, Observer {
            when (it) {
