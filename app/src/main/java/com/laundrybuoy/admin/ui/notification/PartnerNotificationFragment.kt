...
@AndroidEntryPoint
class PartnerNotificationFragment : BaseFragment() {

    private var _binding: FragmentPartnerNotificationBinding? = null
    ...
    private fun initObserver() {
        viewLifecycleOwner.lifecycleScope.launch {
            homeViewModel.getNotificationListing("partner").observe(viewLifecycleOwner) {
    ...
    private fun initRv() {
        ...
        binding.partnerNotificationRv.layoutManager =
            LinearLayoutManager(requireContext(), LinearLayoutManager.VERTICAL, false)
        binding.partnerNotificationRv.setHasFixedSize(true)
        binding.partnerNotificationRv.adapter = notificationListAdapter.withLoadStateHeaderAndFooter(
        ...
        notificationListAdapter.addLoadStateListener { loadState ->
            ...
            if (loadState.source.refresh is LoadState.NotLoading && loadState.append.endOfPaginationReached && notificationListAdapter.itemCount < 1) {
                ...
                binding.partnerNotificationRv.makeViewGone()
            } else {
                binding.ordersUnAvailableLl.makeViewGone()
                binding.partnerNotificationRv.makeViewVisible()
            }
        }
    }
    ...
    override fun onCreateView(
        ...
        _binding = FragmentPartnerNotificationBinding.inflate(inflater, container, false)
        return binding.root
    }
