...
class MarkerInfoWindowAdapter(private val context: Context) : GoogleMap.InfoWindowAdapter {
    ...
    override fun getInfoContents(marker: Marker): View? {
        // 1. Get tag
        val place = marker.tag as? MapInfo2 ?: return null

        // 2. Inflate view and set title, address and rating
        val view = LayoutInflater.from(context).inflate(R.layout.marker_info_contents, null)
        view.findViewById<TextView>(R.id.text_view_title).text = place.name.capitalize()
        view.findViewById<TextView>(R.id.text_view_address).text = place.role.capitalize()
        return view
    }
